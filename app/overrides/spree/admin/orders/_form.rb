Deface::Override.new(
    ...
    text: "<%= render partial: \"spree/admin/orders/shipment\", collection: (current_spree_vendor ? @order.shipments.for_multi_vendor(current_spree_user).order(:created_at) : @order.shipments.order(:created_at)), locals: {order: order} %>"
)
